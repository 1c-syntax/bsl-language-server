...
```
4. Разыменование ссылочных полей в виртуальных таблицах
```bsl
ИЗ
...
```
5. Конструкция "ВЫРАЗИТЬ" с разыменованием получаемого поля
```bsl
ВЫРАЗИТЬ(ВТ_ПланОтгрузок.ДокументПлан КАК Документ.ЗаказКлиента).Валюта.Наценка
```
7. Разыменование ссылочных полей в секции "ГДЕ"
```bsl
ГДЕ
