...
Помимо членов диагностика проверяет конструирование устаревшего типа - `Новый ИмяТипа(...)`. Версия устаревания и рекомендуемые замены берутся с главной страницы типа в синтакс-помощнике, поэтому такая проверка работает только при установленной платформе 1С. Форма `Новый("ИмяТипа")` не проверяется: имя типа там вычисляется в рантайме.
...
Конструирование устаревшего типа. Тип `ЗащищенноеСоединениеNSS` помечен в синтакс-помощнике устаревшим с версии `8.3.8`, рекомендуемая замена - `ЗащищенноеСоединениеOpenSSL`:

```bsl
// Срабатывание: тип устарел с 8.3.8
Соединение = Новый ЗащищенноеСоединениеNSS(СертификатКлиента, СертификатыУдостоверяющихЦентров);
```

Исправление - перейти на рекомендуемую замену:

```bsl
Соединение = Новый ЗащищенноеСоединениеOpenSSL(СертификатКлиента, СертификатыУдостоверяющихЦентров);
```
