...
Кроме членов, диагностика проверяет конструирование самого типа - `Новый ИмяТипа(...)`. Версия появления типа берётся с его главной страницы в синтакс-помощнике, поэтому такая проверка работает только при установленной платформе 1С. Форма `Новый("ИмяТипа")` не проверяется: имя типа там вычисляется в рантайме.
...
Конструирование типа, которого нет в целевой версии. Тип `ЧтениеJSON` доступен только начиная с `8.3.6`, а целевая версия платформы - `8.3.5`:

```bsl
// Срабатывание: тип доступен только с версии 8.3.6
Чтение = Новый ЧтениеJSON();
```

Исправление - поднять целевую версию платформы до `8.3.6` или выше либо решить задачу типами, доступными в целевой версии (для разбора JSON до `8.3.6` штатного типа нет, данные читают строковыми средствами):
