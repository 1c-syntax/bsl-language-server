...
- **MCP roots.** Корни, объявленные клиентом через [MCP roots](https://modelcontextprotocol.io/docs/concepts/roots), по-прежнему индексируются автоматически, включая пересинхронизацию по `roots/list_changed`.

!!! warning "MCP roots объявлены устаревшими"
    В ревизии спецификации [2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28/changelog) механизм roots (вместе с sampling и logging) помечен как deprecated, а уведомление `roots/list_changed` из протокола удалено. В качестве замены спецификация предлагает передавать каталоги через параметры инструментов и конфигурацию сервера — это и делают `register_workspace`/`list_workspaces`. Поддержка roots сохраняется как совместимость со старыми клиентами и будет удалена после перехода на новую ревизию протокола.

## Доступные инструменты

| Инструмент | Назначение |
| --- | --- |
| `list_workspaces` | Зарегистрированные рабочие пространства и их `root` для остальных инструментов |
| `register_workspace` | Регистрация каталога проекта как рабочего пространства с индексацией исходников |
