...
2. `register_workspace` с каталогом проекта (тем, где лежат `Configuration.xml`/`src/cf` или исходники OneScript) — если нужного проекта в списке нет. Инструмент индексирует исходники и возвращает `root`; повторная регистрация того же каталога переиндексацию не запускает.
