...
| `type_info` | По имени типа (например, `Массив`) — его свойства и методы с сигнатурами |
...
| `global_methods` | Список методов глобального контекста (например, `СтартовыйСценарий`/`StartupScript`) с сигнатурами и метаинформацией; можно сузить выборку подстрокой имени |
