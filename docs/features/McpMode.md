...
1. `list_workspace_folders` — узнать, что уже зарегистрировано, и получить значения `root`.
...
3. `unregister_workspace_folder` — освободить индекс, когда проект больше не нужен.
...
!!! warning "MCP roots объявлены устаревшими"
    В ревизии спецификации [2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28/changelog) механизм roots (вместе с sampling и logging) помечен как deprecated, а уведомление `roots/list_changed` из протокола удалено. В качестве замены спецификация предлагает передавать каталоги через параметры инструментов и конфигурацию сервера — это и делают `register_workspace_folder`/`list_workspace_folders`. Поддержка roots сохраняется как совместимость со старыми клиентами; по политике жизненного цикла возможностей MCP удалить их могут не раньше чем через 12 месяцев после этой ревизии, а в сервере они уйдут вместе с переходом на неё.

## Доступные инструменты

| Инструмент | Назначение |
| --- | --- |
| `list_workspace_folders` | Зарегистрированные рабочие папки: `root` для остальных инструментов и имя |
