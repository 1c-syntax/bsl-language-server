...
* `showCyclomaticComplexityCodeLens` - `Булево` - показывать цикломатическую сложность метода над определением метода (codeLens). По умолчанию - `true`.
* `computeDiagnosticsTrigger` - `Строка` - триггер для вызова процедуры рассчета диагностик. Допустимые значения - `onType` (при редактировании файла), `onSave` (при сохранении файла), `never` (никогда). По умолчанию - `onSave`.
* `computeDiagnosticsSkipSupport` - `Строка` - пропуск расчета диагностик в зависимости от уровня поддержки (при наличии конфигурации поставщика).  Допустимые значения - `never` (никогда не пропускать расчет), `withSupport` (пропуск модулей на поддержке), `withSupportLocked` (пропуск модулей на поддержке в запретом редактирования). По умолчанию - `never`.
