...
| OneScript libs | `types.oscript.*` | `OScriptLibraryScanner` walks the workspace once (roots from `OScriptLibraryRootResolver`, per-directory reads via `DirContents`) finding `lib.config` manifests and, via `ConventionalLibraryDiscovery`, convention/flat libraries; `LibConfigParser` parses manifests; `OScriptLibraryIndex` registers types via `TypeRegistry` and `GlobalScopeProvider`; `OScriptModuleTypeResolver` fixes `ModuleType.OScriptClass / OScriptModule`. |
