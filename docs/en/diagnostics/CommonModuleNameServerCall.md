...
Common server modules for calling from the client contain server procedures and functions available from the client code. Они составляют клиентский программный интерфейс сервера приложения. They constitute the client interface of the application server.<br>Such procedures and functions are placed in common modules with the following property:
