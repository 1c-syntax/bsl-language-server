...
После определения цели, необходимо придумать уникальный ключ диагностики, под которым она будет добавлена в общий список. Ключ диагностики будет использоваться как базовая часть имени всех файлов, содержимого метрики.
...
Каждая диагностика должна иметь аннотацию класса `@DiagnosticMetadata`, содержащую метаданные диагностики. Подробная информация об аннотациях в [статье][DiagnosticStructure], для выбора нужной необходимо обратиться к [описанию](docs/contributing/DiagnosticTypeAndSeverity.md).
...
**Внимание!**  
Стоит принять за правило, указание комментарями мест, где диагностика должна сработать.

### Написание теста

В каталоге `src/test/java` в пакете `org.github._1c_syntax.bsl.languageserver.diagnostics` надо создать файл класса диагностики. Имя файла образуется по принципу `%КлючДиагностики%` + `DiagnoscticTest.java`. В нашем примере это будет `SemicolonPresenceDiagnosticTest.java`.  
...
Согласно упомянутой грамматике, для нашего примера необходимо анализировать узлы с типом `statement`, следовательно используем визитер `visitStatement`. Продолжаем, каждый выбранный нами узел должен содержать конечный точку с запятой, представленную токеном `SEMICOLON`.

Таким образом, проверка будет состоят в том, чтобы в каждом узле `statement` получить токен `SEMICOLON`, а если его нет, то зарегистрировать замечание.
...
Файлы должны содержать подробное описание диагностики, примеры `правильно` и `не правильно`, ссылку на источники, если диагностика реализует проверку некотого стандарта, а также описание параметров диагностики. Не мало важной частью описания является блок, описывающий не диагностируемые исключения.
