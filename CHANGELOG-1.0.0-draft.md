...
* **Пользовательские коллекции.** Классы OneScript с аннотацией `&Обходимое` регистрируются как
  коллекции — выводится тип элементов при обходе `Для Каждого`.
...
Поверх новой системы типов добавлена поддержка DI-фреймворка
[«ОСень» (Autumn)](https://github.com/autumn-library/autumn) для OneScript: сервер распознаёт
аннотации фреймворка, индексирует бины и точки внедрения и выводит типы внедряемых зависимостей —
благодаря чему на компонентах «ОСени» работают автодополнение, всплывающие подсказки и переходы.

* **Вывод типов внедряемых зависимостей (DI).** По аннотациям внедрения определяется тип
  внедряемого компонента (`AutumnComponentInferencer`), индексируются бины (`AutumnBeanIndex`) и
  точки внедрения (`AutumnInjectionPointIndex`).
* **Аннотации компонентов и внедрения** (`&Лог`, `&Контроллер`, killjoy и др.) распознаются и
  резолвятся в соответствующие типы.
* **`&ПсевдонимДля` (`AliasFor`)** учитывается при разрешении типов внедряемых зависимостей.
* **Коллекции `autumn-collections`** — выводится тип элементов прилепляемой коллекции.
...
* **Исключение путей из анализа** — параметр `excludePaths` в
  [конфигурационном файле](https://1c-syntax.github.io/bsl-language-server/features/ConfigurationFile):
  список glob-паттернов (например, `**/.git/**`, `build/`, `*.tmp`), файлы и каталоги по которым не
  анализируются. Действует и в режиме LSP, и при пакетном анализе/форматировании из командной строки.
* **Фильтрация диагностик по авторству Git** — параметр `ignoredAuthors` в секции `diagnostics`
  [конфигурационного файла](https://1c-syntax.github.io/bsl-language-server/features/ConfigurationFile):
  список email-адресов авторов, строки кода которых (по данным `git blame`) исключаются из анализа.
  По умолчанию список пуст — фильтрация по авторству не выполняется. Удобно, чтобы не разбирать
  унаследованный или чужой код и сосредоточиться на замечаниях по своим изменениям.
