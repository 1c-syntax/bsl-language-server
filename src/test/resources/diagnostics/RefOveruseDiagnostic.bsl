...
Процедура Тест26()
    Запрос.Текст =
        "ВЫБРАТЬ
        |   Звеньевой.Ссылка // ошибка, обращение к реквизиту без псевдонима таблицы
        |ИЗ
        |   РегистрСведений.Звеньевые";
КонецПроцедуры

Процедура Тест27()
    Запрос.Текст =
        "ВЫБРАТЬ ПЕРВЫЕ 1
         |  Справочник.Пользователи.ТекущееПодразделение
         |ИЗ
         |  Справочник.Пользователи
         |ГДЕ
         |  Справочник.Пользователи.Ссылка = &Пользователь";
КонецПроцедуры

Процедура Тест28()
    Запрос.Текст =
        "ВЫБРАТЬ
        |ЗаявкаНаХранение.Товары.(
        |    Номенклатура КАК Номенклатура,
        |    Ссылка.Склад КАК Склад // не ошибка
        |) КАК Товары
        |ИЗ
        |    Документ.ЗаявкаНаХранение КАК ЗаявкаНаХранение
        |ГДЕ
        |    ЗаявкаНаХранение.Ссылка = &ДокументОснование
        |";
КонецПроцедуры
...
Процедура Тест29()
    Запрос.Текст =
        "ВЫБРАТЬ
         |  Пользователи.ТекущееПодразделение.Код КАК ТекущееПодразделениеКод,
         |  Пользователи.Ссылка.ТекущееПодразделение.Код КАК СсылкаТекущееПодразделениеКод, // ошибка
         |  Пользователи.ТекущееПодразделение.Ссылка.Код КАК ТекущееПодразделениеСсылкаКод // ошибка
         |ИЗ
...
Процедура Тест30()
    Запрос.Текст =
        ...
        |	ВЫБОР
        |		КОГДА ПользователиДополнительныеРеквизиты.Ссылка.ПометкаУдаления // Не ошибка
        |           ТОГДА ПользователиДополнительныеРеквизиты.Ссылка.ТекущееПодразделение.Ссылка // ошибка
        |		ИНАЧЕ ПользователиДополнительныеРеквизиты.Ссылка.ТекущееПодразделение // Не ошибка
        |	КОНЕЦ КАК Поле1
        |ИЗ
...
Процедура Тест31()
    Запрос.Текст =
    "ВЫБРАТЬ
    |    Справочник31.ТабличнаяЧасть1.Ссылка // ошибка без метаданных
    |ИЗ
    |    Справочник.Справочник1 КАК Справочник31";
КонецПроцедуры
...
Процедура Тест32()
    ...
    |    Справочник32.Ссылка
    |ИЗ
    |    Справочник.Справочник1 КАК Справочник32
    |СГРУППИРОВАТЬ ПО
    |    Справочник32.Ссылка
    |ИМЕЮЩИЕ
    |    КОЛИЧЕСТВО(Справочник32.ТабличнаяЧасть1.Ссылка) > 0"; // ошибка без метаданных
КонецПроцедуры
