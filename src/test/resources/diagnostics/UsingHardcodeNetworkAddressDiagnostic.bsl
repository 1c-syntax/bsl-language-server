...
Тест = "(.*<transportConnector name=\"")(.*)(\"" uri=\""tcp://0.0.0.0:61616\?maximumConnections=1000&amp;wireFormat.maxFrameSize=104857600\""/>)"; //
Тест = "_to_SRVMIX"" uri=""static:(tcp://10.1.4.55:61616)"" duplex=""true"" UserName=""system"" Password=""manager""/>"; //
...
СвойстваНаселенныйПункт.Добавить("2.5.4.7"); // <-- ошибка никак не отличить от IP
...
СтрокаСообщения = НСтр("ru = '%1
  ...
  FoundedElement = HTMLDocument.evaluate(".//img[ancestor::div[@aria-hidden=""False""]]", HTMLDocument.body, null, 0, null).iterateNext();
