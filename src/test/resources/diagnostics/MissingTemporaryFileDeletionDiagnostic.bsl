...
&НаКлиенте
Асинх Процедура ПроверкаДиагностикиАсинхроннымУдалением()

    ИмяФайлаАсинх = ПолучитьИмяВременногоФайла(".properties"); // <-- Ошибки нет, асинхронное удаление
    Данные.Записать(ИмяФайлаАсинх);
    Ждать УдалитьФайлыАсинх(ИмяФайлаАсинх);

    TempFileAsync = GetTempFileName(".properties"); // <-- no error, async deletion
    Данные.Записать(TempFileAsync);
    Await DeleteFilesAsync(TempFileAsync);

    ИмяФайлаПеремещение = ПолучитьИмяВременногоФайла("xml"); // <-- Ошибки нет, асинхронное перемещение
    Данные.Записать(ИмяФайлаПеремещение);
    Ждать ПереместитьФайлАсинх(ИмяФайлаПеремещение, "новыйФайл.xml");
