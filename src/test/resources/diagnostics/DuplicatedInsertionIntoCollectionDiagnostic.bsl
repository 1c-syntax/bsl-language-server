...
Процедура ПереиспользованиеВставленногоЭлемента(Результат)
	...
	ИначеЕсли Результат.Получить("СтатусИзвлеченияТекста") = Перечисления.СтатусыИзвлеченияТекстаФайлов.ИзвлечьНеУдалось Тогда
		СтатусИзвлеченияТекстаСтрока = "ИзвлечьНеУдалось";
	ИначеЕсли Результат.Свойство("СтатусИзвлеченияТекста", ХХХ) Тогда
		СтатусИзвлеченияТекстаСтрока = "ИзвлечьНеУдалось";
	КонецЕсли;
	//Результат.Вставить("СтатусИзвлеченияТекста", СтатусИзвлеченияТекстаСтрока); // TODO не ошибка
КонецПроцедуры
...
Процедура ИзменениеРеквизитаВставленногоОбъекта(Результат)
	...
	//ИменаПромежуточныхВТ.Добавить(ИмяВТСотрудники); // TODO не ошибка
КонецПроцедуры
...
Процедура ИзменениеЗначенияИспользуемогоВКлюче(Результат)
    ...
    //Лист.Данные.Вставить("П000001000102_" + Формат(НомСуффикса, "ЧГ="), "5000"); // TODO ошибка или не искать подобные сложные кейсы??

	Протокол = "HTTP";
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(Протокол));

	Протокол = "HTTPS";
	//Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(Протокол)); // TODO ошибка или не искать подобные сложные кейсы?? - в КА 2.5 их совсем мало
	...
	//ОтборПоСобытию.Добавить(ИмяСобытияОшибкиДляЖурналаРегистрации(ПараметрыРасчета)); // TODO ошибка или не искать подобные сложные кейсы?? - в КА 2.5 их совсем мало
КонецПроцедуры

Процедура ИзменениеЗначенияИспользуемогоВКлюче2(Результат)
    ...
    //КонтекстОтчета.Вставить("Графа", "04"); // TODO ошибка или не искать подобные очень сложные кейсы??
КонецПроцедуры

Процедура РазныйРегистрКлюча(Результат)
    // в большой КА 2.5 всего одна такая ошибка - пусть выдается замечание, иначе реализация усложнится, а профит малый
    ВидОшибки.ОбработчикиНажатия.Вставить("Обратитесь",	ОбработчикОткрытияФормыОбращенияВТехподдержку);
    //ВидОшибки.ОбработчикиНажатия.Вставить("обратитесь", ОбработчикОткрытияФормыОбращенияВТехподдержку); // TODO ошибка или пропускать подобные строки с регистром??
КонецПроцедуры

Процедура ОчисткаКоллекции(Результат)
	МассивСумм = Новый Массив;
	МассивСумм.Добавить("Амортизация");

	МассивСумм.Очистить(); // в большой КА 2.5 таких срабатываний всего два )
	//МассивСумм.Добавить("Амортизация"); // TODO ошибка или пропускать подобное??
КонецПроцедуры
