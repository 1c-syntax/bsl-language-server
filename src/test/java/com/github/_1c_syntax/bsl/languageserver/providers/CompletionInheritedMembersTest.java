...
/**
 * Автодополнение членов экземпляра OneScript-класса, наследующего другой класс
 * через библиотеку {@code extends}: после точки должны подсказываться и
 * собственные, и унаследованные (в т.ч. транзитивно) экспортные члены.
 */
@CleanupContextBeforeClassAndAfterClass
class CompletionInheritedMembersTest extends AbstractServerContextAwareTest {
  ...
  void instanceCompletionIncludesInheritedMembers() {
    ...
    // Курсор сразу после «Объект.» на второй строке.
    params.setPosition(new Position(1, "Объект.".length()));
