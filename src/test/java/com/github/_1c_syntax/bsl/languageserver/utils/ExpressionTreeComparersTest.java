...
@SpringBootTest
public class ExpressionTreeComparersTest {

  @Test
  public void strictEquality(){
    ...
    assertThat(comparer.equal(tree.getLeft(), tree.getRight())).isTrue();

    var andExpr = (BinaryOperationNode)tree.getLeft();
    assertThat(comparer.equal(andExpr.getLeft(), andExpr.getRight())).isFalse();
  ...
  public void transitiveEquality(){
    ...
    assertThat(defaultComparer.equal(tree.getLeft(), tree.getRight())).isFalse();

    var transitiveComparer = new TransitiveOperationsIgnoringComparer();
    assertThat(transitiveComparer.equal(tree.getLeft(), tree.getRight())).isTrue();
