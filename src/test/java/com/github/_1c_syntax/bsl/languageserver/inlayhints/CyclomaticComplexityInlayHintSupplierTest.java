...
class CyclomaticComplexityInlayHintSupplierTest {

  private final static String FILE_PATH = "./src/test/resources/inlayhints/CyclomaticComplexityInlayHintSupplier.bsl";

  @Autowired
  private CyclomaticComplexityInlayHintSupplier supplier;
  ...
  void testGetInlayHints() {
    ...
    assertThat(inlayHints)
      ...
      .anySatisfy(inlayHint -> {
        assertThat(inlayHint.getLabel()).isEqualTo(Either.forLeft("+1"));
        assertThat(inlayHint.getKind()).isEqualTo(InlayHintKind.Parameter);
        assertThat(inlayHint.getPosition()).isEqualTo(new Position(0, 10));
        assertThat(inlayHint.getPaddingRight()).isTrue();
        assertThat(inlayHint.getPaddingLeft()).isNull();
      })
    ;
