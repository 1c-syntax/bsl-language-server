...
class SemanticTokensOptionsTest {
  ...
  @Test
  void testDefaultLambdaMethods() {
    // when
    var options = new SemanticTokensOptions();

    // then
    assertThat(options.getLambdaMethods())
      .hasSize(1)
      .contains("Лямбда.Выражение");
  }

  @Test
  void testParsedLambdaMethods() {
    // given
    var options = new SemanticTokensOptions();

    // when
    var parsed = options.getParsedLambdaMethods();
    ...
    assertThat(parsed.moduleMethodPairs())
      .hasSize(1)
      .containsKey("лямбда");
    assertThat(parsed.moduleMethodPairs().get("лямбда"))
      .contains("выражение");
  }

  @Test
  void testSetLambdaMethods() {
    // given
    var options = new SemanticTokensOptions();
    options.setLambdaMethods(List.of(
      "Лямбда.Выражение",
      "Lambda.Expression",
      "CustomLambda"
    ));
    ...
    assertThat(parsed.localMethods())
      .hasSize(1)
      .contains("customlambda");
    assertThat(parsed.moduleMethodPairs())
      .hasSize(2)
      .containsKeys("лямбда", "lambda");
    assertThat(parsed.moduleMethodPairs().get("лямбда"))
      .contains("выражение");
    assertThat(parsed.moduleMethodPairs().get("lambda"))
      .contains("expression");
  }

  @Test
  void testLambdaMethodsCachingBehavior() {
    // given
    var options = new SemanticTokensOptions();
    options.setLambdaMethods(List.of("Lambda.Expression"));

    // when
    var parsed1 = options.getParsedLambdaMethods();
    var parsed2 = options.getParsedLambdaMethods();

    // then
    assertThat(parsed1).isSameAs(parsed2);

    // Update methods
    options.setLambdaMethods(List.of("Lambda.Predicate"));
    var parsed3 = options.getParsedLambdaMethods();

    // Cache should be updated
    assertThat(parsed3).isNotSameAs(parsed1);
    assertThat(parsed3.moduleMethodPairs().get("lambda"))
      .contains("predicate");
  }
}
