  ...
  private void assertDiagnosticList(List<Diagnostic> diagnostics) {
    ...
    assertThat(diagnostics).hasSize(12)
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(2, 12, 3, 57))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*Вопрос.*ПоказатьВопрос.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(21, 4, 21, 84))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*Предупреждение.*ПоказатьПредупреждение.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(29, 4, 29, 26))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ОткрытьЗначение.*ПоказатьЗначение.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(43, 9, 43, 58))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ВвестиДату.*ПоказатьВводДаты.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(72, 9, 72, 67))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ВвестиЗначение.*ПоказатьВводЗначения.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(103, 9, 103, 50))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ВвестиСтроку.*ПоказатьВводСтроки.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(122, 9, 122, 61))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ВвестиЧисло.*ПоказатьВводЧисла.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(138, 4, 138, 50))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*УстановитьВнешнююКомпоненту.*НачатьУстановкуВнешнейКомпоненты.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(148, 4, 148, 33))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ОткрытьФормуМодально.*ОткрытьФорму.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(159, 20, 159, 56))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*УстановитьРасширениеРаботыСФайлами.*НачатьУстановкуРасширенияРаботыСФайлами.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(172, 20, 172, 62))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*УстановитьРасширениеРаботыСКриптографией.*НачатьУстановкуРасширенияРаботыСКриптографией.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(Ranges.create(186, 4, 186, 88))
        && diagnostic.getMessage().getLeft().matches(".*(модального|modal).*ПоместитьФайл.*НачатьПомещениеФайла.*"));
