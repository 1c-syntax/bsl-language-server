...
class UsingSynchronousCallsDiagnosticTest extends AbstractDiagnosticTest<UsingSynchronousCallsDiagnostic> {
  ...
  void testDontUse() {
    ...
    assertThat(diagnostics)
      ...
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(172, 20, 172, 62))
      ...
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(184, 12, 184, 54))
      ...
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(185, 8, 185, 129))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*Предупреждение.*ПоказатьПредупреждение.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(198, 12, 198, 48))
      ...
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(199, 8, 199, 109))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*Предупреждение.*ПоказатьПредупреждение.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(214, 4, 214, 88))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*ПоместитьФайл.*НачатьПомещениеФайла.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(225, 4, 225, 68))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*КопироватьФайл.*НачатьКопированиеФайла.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(236, 4, 236, 69))
      ...
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(260, 8, 260, 37))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*УдалитьФайлы.*НачатьУдалениеФайлов.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(274, 4, 274, 29))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*СоздатьКаталог.*НачатьСозданиеКаталога.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(285, 16, 285, 40))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*КаталогВременныхФайлов.*НачатьПолучениеКаталогаВременныхФайлов.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(296, 16, 296, 35))
      ...
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(318, 16, 318, 89))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*ПолучитьФайлы.*НачатьПолучениеФайлов.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(344, 16, 344, 64))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*ПоместитьФайлы.*НачатьПомещениеФайлов.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(368, 12, 368, 59))
        && diagnostic.getMessage().matches(".*(синхронного|synchronous).*ЗапроситьРазрешениеПользователя.*НачатьЗапросРазрешенияПользователя.*"))
      .anyMatch(diagnostic -> diagnostic.getRange().equals(BSLRanges.create(391, 4, 391, 38))
  ...
  void testUse() {
    ...
    BSLDocumentContext documentContext = getDocumentContextWithUseFlag(UseMode.USE);
    List<Diagnostic> diagnostics = getDiagnostics(documentContext);
    assertThat(diagnostics).isEmpty();
  }

  private BSLDocumentContext getDocumentContextWithUseFlag(UseMode useMode) {
