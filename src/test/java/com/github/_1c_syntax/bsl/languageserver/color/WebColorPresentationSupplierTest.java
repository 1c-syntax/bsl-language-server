...
class WebColorPresentationSupplierTest {
  ...
  @BeforeEach
  void init() {
    documentContext = TestUtils.getDocumentContextFromFile("./src/test/resources/color/WebColorPresentationSupplier.os");
  }
  ...
  void getColorPresentationRu() {
    ...
    assertThatColorPresentations(colorPresentation)
      .hasSize(1)
      .hasLabelAndTextEdit("Через WebЦвета", new TextEdit(params.getRange(), "WebЦвета.Черный"))
    ;
  }
  ...
  void getColorPresentationEn() {
    ...
    assertThatColorPresentations(colorPresentation)
      .hasSize(1)
      .hasLabelAndTextEdit("Via WebColors", new TextEdit(params.getRange(), "WebColors.Black"))
    ;
  }

  @Test
  void getUnknownColor() {
    // given
    var params = new ColorPresentationParams();
    params.setTextDocument(new TextDocumentIdentifier(documentContext.getUri().toString()));
