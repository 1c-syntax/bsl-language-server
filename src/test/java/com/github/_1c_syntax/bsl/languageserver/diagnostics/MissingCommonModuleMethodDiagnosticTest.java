...
class MissingCommonModuleMethodDiagnosticTest extends AbstractDiagnosticTest<MissingCommonModuleMethodDiagnostic> {
  ...
  void test() {
    ...
    assertThat(diagnostics, true)
      ...
      .hasMessageOnRange("Нельзя обращаться к закрытому, неэкспортному методу РегистрацияИзмененийПередУдалением общего модуля ПервыйОбщийМодуль", 11, 22, 56)
      .hasMessageOnRange("Нельзя обращаться к закрытому, неэкспортному методу Тест общего модуля ПервыйОбщийМодуль", 12, 26, 30)
      .hasMessageOnRange("Нельзя обращаться к закрытому, неэкспортному методу Тест общего модуля ПервыйОбщийМодуль", 13, 22, 26)
      .hasMessageOnRange("Нельзя обращаться к закрытому, неэкспортному методу Тест общего модуля ПервыйОбщийМодуль", 14, 22, 26) // TODO не находит https://github.com/1c-syntax/bsl-language-server/issues/2832
      .hasMessageOnRange("Нельзя обращаться к закрытому, неэкспортному методу Тест общего модуля ПервыйОбщийМодуль", 15, 26, 30)
      .hasSize(10);
  }
