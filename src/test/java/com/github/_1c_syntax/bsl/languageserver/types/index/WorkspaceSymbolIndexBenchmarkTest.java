 ...
 * запросов (пустой, короткий префикс полного имени, слово из середины имени, редкий длинный префикс,
 * многословные camel-hump) с прогревом и усреднением по {@link #ITERATIONS} итераций. Все пути —
 * чисто древесные, без линейного скана.
 */
@SpringBootTest
@Slf4j
...
class WorkspaceSymbolIndexBenchmarkTest {
  ...
  /**
   * Однословные запросы — древесный префиксный путь (префикс полного имени и начало любого слова).
   */
  private static final List<String> SINGLE_FRAGMENT_QUERIES = List.of(
    "",        // пустой — вся выдача
    "Пров",    // короткий префикс полного имени
    "Документ", // слово из середины имени — ключевой кейс word-start trie
    "ОбработкаПроведения" // редкий длинный префикс
  );

  /**
   * Многословные camel-hump запросы — древесный путь (пересечение множеств записей по фрагментам).
   */
  ...
  private void benchmarkCorpus(String label, String path) {
    ...
    // when / then — однословные запросы (древесный префиксный путь)
    LOGGER.info("корпус {}: однословные запросы (древесный префиксный путь)", label);
    for (var query : SINGLE_FRAGMENT_QUERIES) {
      benchmarkQuery(query);
    }

    // when / then — многословные camel-hump запросы (древесное пересечение по фрагментам)
    LOGGER.info("корпус {}: многословные camel-hump запросы (древесное пересечение по фрагментам)", label);
