...
public interface InlayHintSupplier<T extends InlayHintData> {
  ...
  /**
   * Ключи конфигурации, управляющие включением сапплаера, в порядке убывания приоритета.
   * <p>
   * Первый из них, присутствующий в {@code inlayHint.parameters}, определяет, включён ли
   * сапплаер. По умолчанию — единственный ключ, совпадающий с {@link #getId()}. Сапплаеры,
   * объединённые под общим ключом конфигурации (вызовы методов), переопределяют метод,
   * возвращая сначала общий ключ, а затем legacy-ключи для обратной совместимости.
   *
   * @return Список ключей конфигурации в порядке убывания приоритета.
   */
