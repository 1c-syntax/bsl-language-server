...
public record TypeSet(
  ...
  /**
   * Объединение двух множеств типов: декорации (element/field) обоих наборов
   * сохраняются, при пересечении ref union-ятся per-key.
   * <p>
   * Вглубь слияние идёт не дальше {@link #MAX_MERGE_DEPTH} уровней вложенности
   * декораций. На этом уровне значения по совпавшим ключам не сливаются: остаётся
   * значение набора-получателя, у второго набора берутся только те ключи, которых
   * у получателя нет. Сами типы ({@link #refs()}) сливаются на любой глубине.
   *
   * @param other второй набор.
   * @return объединение наборов.
   */
