...
public class TypeService implements SelfMemberClassifier {
  ...
  /**
   * Типы, подмешанные в тип расширением, в порядке регистрации. Синтетические типы
   * форм ({@code ТаблицаФормы.<mdoRef>.<имя элемента>}) наследуют состав базового
   * вида данных через расширение — по нему можно отличить, что именно отображает
   * таблица формы.
   *
   * @param ref тип.
  ...
  public List<TypeRef> extensionsOf(TypeRef ref) {
    return typeRegistry.extensionsOf(ref);
  }
