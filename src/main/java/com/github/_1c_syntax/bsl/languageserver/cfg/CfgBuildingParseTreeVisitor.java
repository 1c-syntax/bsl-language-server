...
public class CfgBuildingParseTreeVisitor extends BSLParserBaseVisitor<ParseTree> {
  ...
  public ParseTree visitTryStatement(BSLParser.TryStatementContext ctx) {
    ...
    var exceptionBlock = null;
    CfgVertex exceptionHandler = blocks.getCurrentBlock().getJumpContext().exceptionHandler;

    if (ctx.exceptCodeBlock() != null) {
      blocks.enterBlock();
      ctx.exceptCodeBlock().accept(this);
      exceptionBlock = blocks.leaveBlock();
      exceptionHandler = exceptionBlock.begin();
    }
    ...
    if (exceptionBlock != null) {
      ...
      blocks.getCurrentBlock().getBuildParts().push(exceptionBlock.end());
    } else {
      // If there's no except block, connect the false branch to the parent's exception handler
      graph.addEdge(tryBranch, exceptionHandler, CfgEdgeType.FALSE_BRANCH);
    }
