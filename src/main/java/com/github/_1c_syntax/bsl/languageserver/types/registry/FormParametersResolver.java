...
public class FormParametersResolver {
  ...
  // TODO mdclasses#652: параметр `Основание` объявлен в синтакс-помощнике как
  //  «Произвольный», хотя допустимые типы ограничены списком `ВводитсяНаОсновании`
  //  объекта-владельца формы. Как только свойство появится в релизе mdclasses
  //  (PR 1c-syntax/mdclasses#659), сузить тип по нему — достраивается здесь же,
  //  рядом с `ЗначениеКопирования`, но список зависит от владельца, поэтому
  //  подставлять придётся на стороне формы, как имя в плейсхолдере.
