...
- **`ReferenceIndexFiller`** — обходит ANTLR-дерево (`BSLParserBaseVisitor`) и наполняет индекс.
  ...
  **Обработчик, переданный именем-строкой** (`ПодключитьОбработчикОжидания("Х", 60)`,
  `Элементы.Товары.УстановитьДействие("ПриИзменении", "Х")`), — тоже вхождение: другого
  обращения к такой процедуре в модуле нет, и без него она выглядит никем не вызываемой,
  а переименование её не задевает. Список таких вызовов и позиция имени в них — в
  `utils/AttachedHandlers`; `ОписаниеОповещения` разбирается отдельно
  (`utils/NotifyDescription`): там имён два и у каждого свой модуль.
