...
- **`inferencer/`** — вывод типов. **`ExpressionTypeInferencer`** (workspace-scoped) — `infer()`
  ...
  **`GuardConditionNarrowing`** — сужение на рёбрах веток условия: `ТипЗнч(Х) = Тип("Имя")`
  ...
  выражений, поэтому понимается и отрицание (`Не Х = Неопределено`): под ним конъюнкция и
  дизъюнкция меняются местами по законам де Моргана. Конъюнкция сужает, дизъюнкция — нет;
  на ложной ветке сужает только условие из одной проверки. Охранное предложение
  (`Если … Тогда Возврат; КонецЕсли;`) получается само: до кода за условием доходит только
  ложная ветка. Сужение внутри самой конъюнкции не работает: в `Если Х <> Неопределено И Х.Поле = 5`
  второй операнд не видит `Х` суженным первым — модель сужает по рёбрам, а не внутри условия.
