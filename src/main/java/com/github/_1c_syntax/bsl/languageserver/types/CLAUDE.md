...
- **параметры формы** (`Форма.Параметры`) — тип `ДанныеФормыСтруктура.<mdoRef>`, наполняется
  `ContextType.formParameters()` (bsl-context 0.8.0+) с базового типа и расширения;
  плейсхолдер в типе параметра (`Ключ` объявлен как `СправочникСсылка.<Имя справочника>`)
  подставляется именем владельца формы. В JSON-фолбэке параметров нет;
- **реквизиты — в типах данных формы, а не в прикладных.** На управляемой форме за
  ...
  `СписокЗначений`, `ДинамическийСписок` переносятся как есть. Под объектные типы
...
Ограничения mdclasses 0.20.0 (заведены ишью): не разбираются `<Events>` **внутри элементов**
формы, поэтому обработчики элементов (`…ПриИзменении`, `…Нажатие`) обработчиками не считаются;
не отдаются команды формы и объявленные в форме параметры, поэтому в `Параметры` попадают
только стандартные платформенные.
