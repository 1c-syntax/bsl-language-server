...
- **`inferencer/`** — вывод типов. **`ExpressionTypeInferencer`** (workspace-scoped) — `infer()`
  ...
  **`VariableFlowAnalyzer`** — тип переменной **в точке использования**: расчёт по графу потока
  ...
  Про вывод типов не знает: присваиваемые типы получает колбэком, места присваиваний — списком
  позиций. Применяется в `identifierType`; неприменим (и тогда работает объединение по всем
  присваиваниям из `inferVariable`) для переменных модуля, использования из другого документа
  и присваиваний, которых нет в графе отдельным оператором (связывание в `Для Каждого`).
