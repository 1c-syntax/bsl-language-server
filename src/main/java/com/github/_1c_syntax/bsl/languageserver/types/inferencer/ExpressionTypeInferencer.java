...
public class ExpressionTypeInferencer {
   ...
   * <p>
   * На рекурсивном ребре — том, что ведёт к методу, расчёт которого уже идёт, — берётся
   * значение из индекса, а не пустота. Пустота делала бы результат зависящим от того, с
   * какой стороны вошли в цикл: посчитанное первым звено видело бы вместо соседа ничто,
   * а посчитанное вторым — уже его значение. Чтение индекса от точки входа не зависит,
   * поэтому расчёт становится функцией от того, что известно, и повторение доводит цикл
   * до общего для всех порядков предела.
   */
  private TypeSet methodReturnType(MethodSymbol method, InferenceContext ctx) {
    if (!ctx.visited.add(method)) {
      // Значение рекурсивного ребра меняется от повторения к повторению, поэтому результат,
      // на нём построенный, в кэш выражений не годится (см. inferInternal).
      ctx.cycleCut = true;
