...
final class FormPlatformTypes {
  ...
  enum TableDataKind {
     ...
     * данных, где своей строки нет: части компоновщика настроек и отбор отдают
     * <b>копию</b> данных обычной {@code Структура}, диаграмма Ганта —
     * {@code ДанныеФормыСтруктура} с колонками диаграммы.
     * <p>
     * Слову «структура» из описания расширения верить нельзя: у таблицы над данными
     * формы оно означает не тип {@code Структура}, а «структуроподобный объект» —
     * {@code ТипЗнч(Элементы.ТабличнаяЧасть1.ТекущиеДанные)} на самом деле даёт
     * {@code ДанныеФормыЭлементКоллекции}. Поэтому там, где строка есть, берётся она.
     * У компоновщика настроек своей строки нет вовсе, и описание — единственный источник.
     *
    ...
    @Nullable String currentDataTypeName() {
      return switch (this) {
        ...
        case GANTT_CHART -> FORM_DATA_STRUCTURE_RU;
        default -> "Структура";
      };
    }
