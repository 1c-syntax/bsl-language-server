...
final class FormPlatformTypes {
   ...
   * документирует оба написания ({@code СтрокаПоиска (SearchString)}). В структуру же
   * попадает <b>одно</b> — то, что соответствует варианту встроенного языка проекта:
   * в русской конфигурации английского написания там не будет. Поэтому из пары берётся
   * написание по языку проекта, а не оба сразу.
   *
