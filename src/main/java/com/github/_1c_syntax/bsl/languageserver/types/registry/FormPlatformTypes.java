...
final class FormPlatformTypes {
  ...
  enum TableDataKind {
     ...
     *
     * @return имя типа; {@code null} — описание типа не называет («идентификатор строки»,
     *   «идентификатор элемента»), и выдумывать его нельзя.
     */
    @Nullable String rowIdTypeName() {
      return switch (this) {
        case GANTT_CHART -> "ИдентификаторЗначенияДиаграммыГанта";
        case DYNAMIC_LIST, TABULAR_SECTION, VALUE_TABLE, VALUE_TREE, VALUE_LIST, FILTER -> null;
     ...
     *   берётся оттуда: там видны колонки, а обобщённая структура их скрыла бы.
     */
